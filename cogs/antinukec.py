...
from core import Soward, Context
import discord
...
class Antinuke(Cog):
  ...
  async def antinuke_enable(self, ctx: Context):
    ...
    if ctx.author.id == ctx.guild.owner_id:
      if data == "on":
        ...
        embed2.set_footer(text=f"Soward", icon_url="https://images-ext-2.discordapp.net/external/vbj6ibvLs55RTX2t0uvyX54lJw3BRSlIMUEOYyAFg4o/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/1004248513435152484/64dea1f7b0728ca6153a037725bdd037.png")
  ...
  async def antinuke_disable(self, ctx: Context):
    ...
    if ctx.author.id == ctx.guild.owner_id:
      if data == "off":
        emb = discord.Embed(title="**__Soward__**", description=f" Antinuke already disabled in this server\nCurrent Status: Disabled <:off:1031276015152021604>\n\nTo enable Antinuke use `antinuke enable`", color=0x2f3136, timestamp=ctx.message.created_at)
        emb.set_author(name=ctx.author, icon_url=ctx.author.display_avatar.url)
        emb.set_thumbnail(url=ctx.guild.icon)
        emb.set_footer(text="Soward", icon_url="https://images-ext-2.discordapp.net/external/vbj6ibvLs55RTX2t0uvyX54lJw3BRSlIMUEOYyAFg4o/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/1004248513435152484/64dea1f7b0728ca6153a037725bdd037.png")
        ...
        swrd.set_footer(text="Soward", icon_url="https://images-ext-2.discordapp.net/external/vbj6ibvLs55RTX2t0uvyX54lJw3BRSlIMUEOYyAFg4o/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/1004248513435152484/64dea1f7b0728ca6153a037725bdd037.png")
  ...
  async def antinuke_config(self, ctx: Context):
    ...
    if data == "off":
      ...
      embed2.add_field(name=f"**__Current punishment type__**" , value=f"{punish}")
      embed2.set_author(name=ctx.author, icon_url=ctx.author.display_avatar.url)
      embed2.set_footer(text="Soward", icon_url="https://images-ext-2.discordapp.net/external/vbj6ibvLs55RTX2t0uvyX54lJw3BRSlIMUEOYyAFg4o/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/1004248513435152484/64dea1f7b0728ca6153a037725bdd037.png")
    ...
    for channel in ctx.guild.channels:
        if channel.name in ('rules', 'moderator-only'):
            try:
                ...
                pass
    await ctx.reply(embed=discord.Embed(title=f"Successfully Deleted All Channels With Name Of `rules, moderator-only`", mention_author=False), color=0x2f3136)
  ...
  async def punishment_set(self, ctx, punishment: str):
        ...
        if ctx.author == owner:
            ...
            if kickOrBan == "kick":
                ...
                await ctx.reply(embed=discord.Embed(title=f"Punishment set To **{kickOrBan}**", color=0x2f3136))
                ...
                await ctx.reply(embed=discord.Embed(title=f"Punishment Changed To **{kickOrBan}**", color=0x2f3136))

                updateConfig(ctx.guild.id, data)
            else:
               await ctx.reply(embed=discord.Embed(title=f"Invalid Punishment Type\nValid Punishment  Kick, Ban", color=0x2f3136))

        else:
            await ctx.reply(embed=discord.Embed(title="This Command Can Only be Executed By This Server\'s Owner", color=0x2f3136))
  ...
  async def punishment_show(self, ctx: Context):
    ...
    await ctx.reply(embed=discord.Embed(title="Antinuke punishment in this server is: **{}**".format(punish.title(), color=0x2f3136)))
 ...
 # @commands.guild_only()
    ...
    if ctx.author == owner:
      if len(wled) == 10:
        ...
        if str(user.id) in wled:
          await ctx.reply(embed=discord.Embed(title="That user is already  whitelisted!", color=0x2f3136))
        else:
          wled.append(str(user.id))
          updateConfig(ctx.guild.id, data)
          await ctx.reply(embed=discord.Embed(title=" **{}** has been added to the whitelist".format(user), color=0x2f3136))

    else:
          await ctx.reply(embed=discord.Embed(title="You must be guild owner to whitelist someone  ", color=0x2f3136))
  ...
  async def whitelist_remove(self, ctx, user: discord.User):
    ...
    if ctx.author == owner:
      if str(user.id) in wled:
        ...
        updateConfig(ctx.guild.id, data)
        await ctx.reply(embed=discord.Embed(title="**{}** has been removed from the whitelist".format(user), color=0x2f3136))
      else:
        await ctx.reply(embed=discord.Embed(title="That user was never whitelisted", color=0x2f3136))
    else:
      await ctx.reply(embed=discord.Embed(title="You must be the guild owner to remove someone from whitelist", color=0x2f3136))
  ...
  async def wl_reset(self, ctx: Context):
    if ctx.author.id == ctx.guild.owner_id:
      ...
      data["whitelisted"] = []
      updateConfig(ctx.guild.id, data)
      await ctx.reply(embed=discord.Embed(title="Successfully Removed all whitelisted users ", color=0x2f3136))
    else:
