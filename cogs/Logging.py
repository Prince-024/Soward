...
class logging(commands.Cog):
    ...
    async def channel(self, ctx, c: discord.TextChannel):
        try:
            ...
            await ctx.send(f"<:tick_right:1003345911067443241>  | All logs channel are updated to <#{c.id}>")
    ...
    async def config(self, ctx):
        ...
        x = data["log-channel"]
        if x == None:
          embed = discord.Embed(title=f"Logging channel:", description=f"No Logging Channel Found", color = 0x13dd4e)
          await ctx.send(embed=embed)
        else:
          embed = discord.Embed(title=f"Logging channel:", description=f"<#{x}>", color = 0x13dd4e)
          await ctx.send(embed=embed)
    ...
    async def delete(self, ctx: commands.Context):
        ...
        await ctx.send(f'<:tick_right:1003345911067443241> | Successfully Deleted Logging Channel')
    ...
    async def on_guild_emoji_create(self, emoji):
        with open("./data/databases/logging.json") as f:
            ...
            channel = self.bot.get_channel(x)
            embed = discord.Embed(description=f"Emoji ({emoji}) has been added.", color = 0x2f3136)
