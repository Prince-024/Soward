...
headers = {
    'Authorization':
    f'OTkwMjU3MTMwMTIyNjUzNzM2.GAx4Kb.xA8vaPXmXc3yyjwqAkDN-pk2RpJFtkbAytongE'
}
...
async def help(ctx):
    ...
    b = Button(label='Invite Me',
               style=discord.ButtonStyle.link,
               url='https://dsc.gg/ethrex')
    view = View()
    view.add_item(b)

    b3 = Button(label='support',
                style=discord.ButtonStyle.link,
                url='https://discord.gg/idk')
    view = View()
    view.add_item(b)
    #  view.add_item(b2)
    view.add_item(b3)
    embed = discord.Embed(
        title="Help command",
        description=
        f"Discord Multipurpose bot with  Moderation, ticket, information, utility and more.default Prefix = @ \n\n  [Get Ethrex](https://discord.com/oauth2/authorize?client_id=863262252211765248&permissions=1101052116095&scope=applications.commands%20bot) | [Support](https://discord.gg/r-one)",
        color=0xFF1B1B,
        timestamp=ctx.message.created_at)
    embed.set_thumbnail(url=bot.user.avatar)
    ...
    embed.set_footer(text=f"Made By Harsh.ly?", icon_url=bot.user.avatar)
    embed.add_field(
        name="Ethrex ",
        value=
        f"<:ares_bank:1076468010254094416> __** COMMAND MODULE**__**\n\n<:ares_star_badge:1076463498617438271> • **Security**\n<:ares_hammer:1076463242118971434> •  **Moderation**\n<:ares_box:1076465300763050024> • **Utility**\n<a:_ticket:1076461829288640522> • **Ticket**\n<:ares_award:1076466403206504541> • **Welcomes**\n<:ares_monitor:1076463916458188941> • **Economy**\n<:ares_plus:1076464025476546560> • **Extra**\n<:ares_mic:1076464329093820426> • **Join2Crete**\n<:ares_people:1076464910164303912> • **Autorole**\n<:ares_plant:1076467174807453727> • **Games**\n<:ares_stagerequesttospeaklist:1076465107388858520> • **General**\n<:ares_award:1076466403206504541> • **Fun**\n<:ares_eventcolour:1076465472452710561> • **Selfrole\n\n``` $<module> ex:$welcomes```",
        inline=True)
...
async def antilink(ctx, toggle):
    ...
    if toggle == "enable":
        ...
        await ctx.reply(
            embed=discord.Embed(title=f" enabled antilink ", color=0xFF1B1B))
    elif toggle == "disable":
        idk[str(ctx.guild.id)] = "disable"
        await ctx.reply(
            embed=discord.Embed(title=" Disabled antilink ", color=0xFF1B1B))
    else:
        await ctx.reply(embed=discord.Embed(
            title=f"Wrong ❌ Invalid argument, it should be enable or disable.",
            color=0xFF1B1B))
...
async def reboot(ctx):
    await ctx.send(embed=discord.Embed(
        title=" **Successfully Restarted The Bot**", color=0xFF1B1B))
    restart_client()
...
async def on_command_error(ctx, error):
    ...
    if type(error) == commands.MissingRequiredArgument:
        return await ctx.send(embed=discord.Embed(
            title="Error",
            description=
            f"You forgot to provide an argument, please do it like: `{ctx.command.name} {ctx.command.usage}`",
            color=0xFF1B1B),
                              delete_after=5)
    elif type(error) == commands.BotMissingPermissions:
        return await ctx.send(embed=discord.Embed(
            title=f"The bot is missing Permissions:",
            description=f" {', '.join(error.missing_perms)}",
            color=0xFF1B1B))
...
chatbot = discord.Embed(
    description=
    f'**```  Antinuke enable/disable ・ Antinuke config ・ features ・ whitelist add・ whitelist remove・ whitelist show ・ whitelist reset ・ channelclean ・ roleclean ・ recover ・ punishment set ・ punishment show```**'
)
mod = discord.Embed(
    description=f'**``` ar-create ・ ar-delete ・ ar-show ・ ar-edi ```**')
utility = discord.Embed(
    description=
    f'**``` Bal ・ dep ・ with ・ Rob ・ Slots ・ Shop ・ Buy ・ Sell ・ Beg ・ Work ・ Rich ・ Send```**'
)
games = discord.Embed(
    description=
    ...
    select = Select(
        ...
        options=[
            discord.SelectOption(
                label='economy',
                description='Full list of commands for chatbot',
                value='12',
                emoji=f'<a:cash:1017403803726778409>'),
            discord.SelectOption(
                label='Utility Commands',
                description='Full list of commands for utility',
                value='14',
                emoji=f'<:utility:1017405604987404408>'),
            discord.SelectOption(
                label='Moderation',
                description='Full list of commands for mods',
                value='15',
                emoji=f'<a:moderation_animated:1017402486258151534>'),
            discord.SelectOption(label='Games',
                                 description='full list of games commands',
                                 value='16',
...
async def autoresponse_create(ctx, name, *, message):
    ...
    if str(ctx.guild.id) in autoresponse:
        ...
        if len(numbers) >= 10:
            return await ctx.send(embed=discord.Embed(
                title=f'You can\'t add more than 10 autoresponses in a server',
                color=0xFF1B1B))
    if str(ctx.guild.id) in autoresponse:
        if name in autoresponse[str(ctx.guild.id)]:
            return await ctx.send(embed=discord.Embed(
                title=f' The autoresponse `{name}` is already in the server',
                color=0xFF1B1B))
    if str(ctx.guild.id) in autoresponse:
        ...
        return await ctx.reply(embed=discord.Embed(
            title=f'Created a autoresponse with the name : `{name}`',
            color=0xFF1B1B))
    ...
    if str(ctx.guild.id) in autoresponse:
        if name in autoresponse[str(ctx.guild.id)]:
            ...
            with open("autoresponse.json", "w") as f:
                json.dump(autoresponse, f, indent=4)
            return await ctx.reply(embed=discord.Embed(
                title=f'Deleted the `{name}` autoresponse in the server',
                color=0xFF1B1B))
        else:
            return await ctx.reply(embed=discord.Embed(
                title=f'No autoresponse found with the name `{name}`',
                color=0xFF1B1B))
    else:
        return await ctx.reply(embed=discord.Embed(
            title=f'There is no autoresponses in the server', color=0xFF1B1B))
    ...
    if str(ctx.guild.id) in autoresponse:
        if name in autoresponse[str(ctx.guild.id)]:
            ...
            with open("autoresponse.json", "w") as f:
                json.dump(autoresponse, f, indent=4)
            return await ctx.send(embed=discord.Embed(
                title=f'Edited the `{name}` autoresponse', color=0xFF1B1B))
    else:
        return await ctx.send(embed=discord.Embed(
            title=f'There is no autoresponses in the server', color=0xFF1B1B))
...
async def vckick(ctx, member: discord.Member, reason="No reason provided"):
    ...
    await ctx.reply(embed=discord.Embed(
        title=f' | {member} has been disconnected from vc.', color=0xFF1B1B))
...
async def vchide(ctx, channel: discord.VoiceChannel = None):
    ...
    if ch == None:
        ...
        await ctx.reply(embed=discord.Embed(
            title=
            f'<:ri8:1038487759750438912> | {ch.mention} is now hidden from the default role.',
            color=0xFF1B1B))
...
async def features(ctx):
    ...
    em.set_footer(
        text="Made By Abhi",
        icon_url=
        "https://images-ext-2.discordapp.net/external/XpYSeN_4K1TG8OtzI3R3LE3zXbhvqB1rwgQkKRSs-Ww/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/884798892934332416/aa3b4e68dd27540854c0e0e3f374fe32.png"
    )
...
async def on_message(message):
    ...
    if prince in message.content:
        ...
        await msg.add_reaction(
            "<a:azzzzzzz_thory_kiddo_heart:934674783180828682>")
...
async def setupfriend(ctx, role: discord.Role = None):
    ...
    with open('friends.json', 'w', encoding='utf-8') as f:
        ...
        await ctx.send(embed=discord.Embed(
            description=
            f"<:ri8:1038487759750438912> Updated Friends Role To {role.mention}",
            color=0xFF1B1B))
...
async def friend(ctx, mem: discord.Member = None):
    ...
    if f'{ctx.guild.id}' not in key:
        await ctx.reply(embed=discord.Embed(title="Friends role is not set!",
                                            color=0xFF1B1B))
        ...
        for idk in key[str(ctx.guild.id)]:
            ...
            await ctx.send(embed=discord.Embed(
                description=
                f'<:ri8:1038487759750438912> Added {r.mention} To {mem.mention}',
                color=0xFF1B1B))
...
async def setupguest(ctx, role: discord.Role = None):
    ...
    with open('guest.json', 'w', encoding='utf-8') as f:
        ...
        await ctx.send(embed=discord.Embed(
            description=
            f"<:ri8:1038487759750438912> Updated Guest Role To {role.mention}",
            color=0xFF1B1B))
...
async def guest(ctx, mem: discord.Member = None):
    ...
    if f'{ctx.guild.id}' not in key:
        await ctx.reply(
            embed=discord.Embed(title="Guest role is not set!", color=0xFF1B1B)
        )
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
...
async def setupofficial(ctx, role: discord.Role = None):
    ...
    with open('official.json', 'w', encoding='utf-8') as f:
        ...
        await ctx.send(embed=discord.Embed(
            description=
            f"<:ri8:1038487759750438912> Updated official Role To {role.mention}",
            color=0xFF1B1B))
...
async def official(ctx, mem: discord.Member = None):
    with open("official.json", 'r') as f:
        key = json.load(f)
    if f'{ctx.guild.id}' not in key:
        await ctx.reply(embed=discord.Embed(title="Official role is not set!",
                                            color=0xFF1B1B))
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
...
async def setupgirl(ctx, role: discord.Role = None):
    ...
    with open('girl.json', 'w', encoding='utf-8') as f:
        ...
        await ctx.send(embed=discord.Embed(
            description=
            f"<:ri8:1038487759750438912> Updated girls Role To {role.mention}",
            color=0xFF1B1B))
...
async def girl(ctx, mem: discord.Member = None):
    ...
    if f'{ctx.guild.id}' not in key:
        await ctx.reply(
            embed=discord.Embed(title="Girls role is not set!", color=0xFF1B1B)
        )
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
...
async def setupvip(ctx, role: discord.Role = None):
    ...
    with open('vip.json', 'w', encoding='utf-8') as f:
        ...
        await ctx.send(embed=discord.Embed(
            description=
            f"<:ri8:1038487759750438912> Updated vip Role To {role.mention}",
            color=0xFF1B1B))
...
async def vip(ctx, mem: discord.Member = None):
    ...
    if f'{ctx.guild.id}' not in key:
        await ctx.reply(
            embed=discord.Embed(title="Vip role is not set!", color=0xFF1B1B))
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
            await mem.add_roles(r)
...
async def rofficial(ctx, mem: discord.Member = None):
    ...
    if f'{ctx.guild.id}' not in key:
        ...
        for idk in key[str(ctx.guild.id)]:
            ...
            await ctx.send(embed=discord.Embed(
                description=
                f'<:ri8:1038487759750438912> Removed {r.mention} from {mem.mention}',
                color=0xFF1B1B))
...
async def rbot(ctx, mem: discord.Member = None):
    ...
    if f'{ctx.guild.id}' not in key:
        await ctx.send(
            embed=discord.Embed(title="bot role is not set!", color=0xFF1B1B))
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
            await mem.remove_roles(r)
...
async def rartist(ctx, mem: discord.Member = None):
    with open("artist.json", 'r') as f:
        key = json.load(f)
    if f'{ctx.guild.id}' not in key:
        await ctx.send(embed=discord.Embed(title="artist role is not set!",
                                           color=0xFF1B1B))
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
...
async def rmod(ctx, mem: discord.Member = None):
    ...
    if f'{ctx.guild.id}' not in key:
        await ctx.send(
            embed=discord.Embed(title="Mod role is not set!", color=0xFF1B1B))
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
            await mem.remove_roles(r)
...
async def sfriend(ctx, mem: discord.Member = None):
    with open("friends.json", 'r') as f:
        key = json.load(f)
    if f'{ctx.guild.id}' not in key:
        await ctx.send(embed=discord.Embed(title="Friends role is not set!",
                                           color=0xFF1B1B))
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
...
async def sofficial(ctx, mem: discord.Member = None):
    with open("official.json", 'r') as f:
        key = json.load(f)
    if f'{ctx.guild.id}' not in key:
        await ctx.send(embed=discord.Embed(title="Officials role is not set!",
                                           color=0xFF1B1B))
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
...
async def setup_config(ctx, mem: discord.Member = None):
    ...
    if f'{ctx.guild.id}' not in key:
        ...
        for idk in key[str(ctx.guild.id)]:
            ...
            artr = f"{ar.mention}"
    embed = discord.Embed(title=f"Role setup for {ctx.guild}",
                          color=0xFF1B1B,
                          timestamp=ctx.message.created_at)
...
async def setupbot(ctx, role: discord.Role = None):
    ...
    with open('bot.json', 'w', encoding='utf-8') as f:
        ...
        await ctx.send(embed=discord.Embed(
            description=
            f"<:ri8:1038487759750438912> Updated bot Role To {role.mention}",
            color=0xFF1B1B))
...
async def setupmod(ctx, role: discord.Role = None):
    ...
    with open('mod.json', 'w', encoding='utf-8') as f:
        ...
        await ctx.send(embed=discord.Embed(
            description=
            f"<:ri8:1038487759750438912> Updated Mod Role To {role.mention}",
            color=0xFF1B1B))
...
async def setupartist(ctx, role: discord.Role = None):
    ...
    with open('artist.json', 'w', encoding='utf-8') as f:
        ...
        await ctx.send(embed=discord.Embed(
            description=
            f"<:ri8:1038487759750438912> Updated Artist Role To {role.mention}",
            color=0xFF1B1B))
...
async def artist(ctx, mem: discord.Member = None):
    with open("artist.json", 'r') as f:
        key = json.load(f)
    if f'{ctx.guild.id}' not in key:
        await ctx.reply(embed=discord.Embed(title="Artist role is not set!",
                                            color=0xFF1B1B))
    elif f'{ctx.guild.id}' in key:
        for idk in key[str(ctx.guild.id)]:
            r = discord.utils.get(ctx.guild.roles, id=int(idk))
...
async def unmute(ctx, member: discord.Member, reason=None):
    ...
    await member.send(
        f":exclamation: | You are have been unmuted from: {ctx.guild.name} by {ctx.author.name}"
    )
    await ctx.send(embed=discord.Embed(
        title=f"<:ri8:1038487759750438912> {member.name} successfully unmuted",
        color=0xFF1B1B))
...
badgesgiver = [
    884798892934332416, 925246550018519040, 940973004647718912,
    971301161678290964, 969991261048164352, 884798892934332416,
    984815117730480228, 990643162928279592
]
...
async def addbadge(ctx, user: discord.Member, *, badge):
    if ctx.author.id in badgesgiver:
        ...
        if str(user.id) not in idk:
            ...
            await ctx.reply(embed=discord.Embed(
                title=f" Added badge {badge} to {user}.", color=0xFF1B1B))
...
async def badges(ctx, user: discord.Member = None):
    ...
    if str(user.id) not in idk:
        await ctx.reply(embed=discord.Embed(
            description=
            f"{user} Have no badges join [support server](https://discord.gg/k6YNHy36JJ) to get some badges.",
            color=0xFF1B1B),
        ...
        embed = discord.Embed(
            ...
            title=
            "<a:hypesquad:1017430505353904128> Ethrex Achivement <a:hypesquad:1017430505353904128> ",
            description=f"{user.mention} badges\n\n")
...
async def removebadge(ctx, user: discord.User = None):
    if ctx.author.id in badgesgiver:
        if user is None:
            await ctx.reply(embed=discord.Embed(
                title="You must specify a user to remove badge.",
                color=0xFF1B1B))
            return
        with open('badges.json', 'r') as f:
            badges = json.load(f)
        try:
            if str(user.id) in badges:
                ...
                await ctx.reply(embed=discord.Embed(
                    title=f"Removed badge of {user}", color=0xFF1B1B))
        except KeyError:
...
async def verification(ctx, verification_channel: discord.TextChannel,
    ...
    embed = discord.Embed(
        title=
        "<a:Error:1018257469274861688> Verification Required <a:Error:1018257469274861688>",
        description=
        f"<a:verify:1033811571031408732> To access **{ctx.guild.name}**\n     <a:verify:1033811571031408732> you need to pass the verification first\n Press the verify button below.",
        timestamp=ctx.message.created_at,
        color=0xFF1B1B)
    embed.set_thumbnail(url=ctx.guild.icon)
    embed.set_footer(text=bot.user.name, icon_url=bot.user.avatar)
...
class selfrole2(discord.ui.View):
    ...
    @discord.ui.button(label='GetRole',
                       style=discord.ButtonStyle.green,
                       custom_id=f'ReactionRole')
    async def button_callback(self, button, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"Failed to Add",
                                                    ephemeral=True)
...
async def botautoroleidk(ctx, role: discord.Role):
    ...
    if me.top_role >= ctx.message.author.top_role and ctx.message.author.id != ctx.guild.owner_id:
        await ctx.reply(embed=discord.Embed(
            title="You must have role above me to use this cmd.",
            color=0xFF1B1B))
        return
    if role.permissions.administrator:
        await ctx.reply(embed=discord.Embed(
            title="You can't use roles with administrator in autoroles.",
            color=0xFF1B1B))
        return
    else:
        with open('roles.json', 'r') as f:
        ...
        if str(role.id) in omk['botautoroles']:
            await ctx.reply(embed=discord.Embed(
                title="That role is already set in autoroles.", color=0xFF1B1B)
                            )
            return
        else:
            ...
            await ctx.reply(embed=discord.Embed(
                title="successfully Added Bot autoroles.", color=0xFF1B1B))
    ...
    if role.permissions.administrator:
        ...
        if str(role.id) in omk['humanautoroles']:
            ...
            with open('roles.json', 'w') as f:
                json.dump(ff, f, indent=4)
            await ctx.reply(embed=discord.Embed(
                title="Added that role to autoroles.", color=0xFF1B1B))
...
async def botarmutoroleidk(ctx, role: discord.Role):
    ...
    if me.top_role >= ctx.message.author.top_role and ctx.message.author.id != ctx.guild.owner_id:
        ...
        if not str(role.id) in omk['botautoroles']:
            await ctx.reply(embed=discord.Embed(
                title="That role is not set in autoroles.", color=0xFF1B1B))
            return
        else:
            ...
            with open('roles.json', 'w') as f:
                json.dump(ff, f, indent=4)
            await ctx.reply(embed=discord.Embed(
                title="Removed that role from  autoroles.", color=0xFF1B1B))
...
async def auto_rolshow(ctx):
    ...
    if human_autoroles == [] and bot_at == []:
        ...
        embed.add_field(name="Human Auto-Roles",
...
async def np(ctx, type=None, mem: discord.Member = None):
    if ctx.author.id not in owners:
        await ctx.reply(embed=discord.Embed(
            title=
            f"<:Wrong:1017402708703064144> | You don't have access to this command !",
            color=0xFF1B1B))
        return
    else:
        if type == None:
            await ctx.reply(embed=discord.Embed(
                title=
                f"<:Wrong:1017402708703064144> | You missed the 'Type' argument",
                color=0xFF1B1B),
                            mention_author=False)
            return
        if type == "add":
            if mem == None:
                await ctx.reply(embed=discord.Embed(
                    title=
                    f"<:Wrong:1017402708703064144> | You missed the 'Member' argument",
                    color=0xFF1B1B),
                                mention_author=False)
                return
            else:
                with open("nonprefix.json", "r") as f:
                    ...
                    if str(mem.id) in member["access"]:
                        await ctx.reply(embed=discord.Embed(
                            title=
                            f"<:Wrong:1017402708703064144> | The mentioned member is already added!",
                            color=0xFF1B1B),
                                        mention_author=False)
                        return
                    else:
                        ...
                        await ctx.reply(embed=discord.Embed(
                            title=
                            f"<:Icons_correct:1017402689027592222> |  successfully added {mem.name} to noprefix !",
                            color=0xFF1B1B),
                                        mention_author=False)
        if type == "remove":
            if mem == None:
                ...
                with open("nonprefix.json", "r") as f:
                    ...
                    if str(mem.id) not in member["access"]:
                        await ctx.reply(embed=discord.Embed(
                            title=
                            f"<:Wrong:1017402708703064144> | The mentioned member is already absent!",
                            color=0xFF1B1B),
                                        mention_author=False)
                        return
                    else:
                        ...
                        await ctx.reply(embed=discord.Embed(
                            title=
                            f"<:Icons_correct:1017402689027592222> | {mem.name} is successfully removed from noprefix!",
                            color=0xFF1B1B),
                                        mention_author=False)
        if type == "list":
        ...
        if str(message.guild.id) not in idk or idk[str(
            ...
            if message.author.guild_permissions.manage_messages:
                ...
                if message.author.id != bot.user.id:
                    ...
                    await message.author.timeout_for(
                        duration, reason="Ethrex | antispam")
...
async def banlist(ctx):
    ...
    if len(list) > 0:
        ...
        embed1 = discord.Embed(title=f'Terminal',
                               url=f"{invitelink}",
                               description=banned,
                               color=0xFF1B1B)
        embed1.set_footer(
        ...
        embed2 = discord.Embed(
            title=f'Ethrex',
            url=f'{invitelink}',
            description="There are no banned users for this guild",
            color=0xFF1B1B)
        embed2.set_footer(
            text=f'Requested by {ctx.author.name}#{ctx.author.discriminator}',
            icon_url=ctx.author.display_avatar.url)
...
async def membercount(ctx):
    ...
    online = len(
        list(filter(lambda m: str(m.status) == "online", ctx.guild.members)))
    idle = len(
        list(filter(lambda m: str(m.status) == "idle", ctx.guild.members)))
    dnd = len(list(filter(lambda m: str(m.status) == "dnd",
                          ctx.guild.members)))
    offline = len(
        list(filter(lambda m: str(m.status) == "offline", ctx.guild.members)))
    t_online = [online, idle, dnd]
    Sum = sum(t_online)
    mbed = discord.Embed(
        title="Membercount",
        description=
        f"**Total Members = {ctx.author.guild.member_count} \n Users = {user_count} \n Bots = {ctx.author.guild.member_count - user_count} \n Total Online = {Sum} \n Online status = {online} \n Idle status = {idle} \n Dnd status = {dnd} \n Offline = {offline}**",
        color=0xFF1B1B)
    mbed.set_author(name=ctx.guild.name,
                    icon_url=ctx.author.display_avatar.url)
    if ctx.guild.icon:
        mbed.set_thumbnail(url=ctx.guild.icon)
        mbed.set_footer(text=f"Requested by {ctx.author.name}",
                        icon_url=ctx.author.display_avatar.url)
...
async def set_greet(ctx, *, msg=None):
    if msg == None:
        await ctx.send(
            embed=discord.Embed(title="pls provide msg !!", color=0xFF1B1B))
        return
    if not "$$MM$$" in msg:
        await ctx.reply(
            f"**use member mention \n example:** \n `greet heyy!! $$MM$$ welcome to {ctx.guild.name} server !!`  "
        )
    else:
        try:
            ...
            write_json(database)
            await ctx.send(embed=discord.Embed(
                title="successfully set welcome msg !!", color=0xFF1B1B),
                           delete_after=5)
...
async def stop_greet(ctx):
    if not str(ctx.guild.id) in welcm.keys():
        await ctx.send(embed=discord.Embed(
            title="in this server not set welcome message !!", color=0xFF1B1B))
        return
    elif not str(ctx.channel.id) in welcm[str(ctx.guild.id)].keys():
        await ctx.send(embed=discord.Embed(
            title="welcome message not set in this channel.", color=0xFF1B1B))
        return
    else:
        try:
            ...
            write_json(database)
            await ctx.send(embed=discord.Embed(
                title="stop welcome message in this channel.", color=0xFF1B1B))
            await ctx.message.delete()
        except:
            pass
...
async def bremove(ctx: Context, member: discord.Member = None):
    try:
        ...
        embed = discord.Embed(
            ...
            description=
            f"**<:Icons_correct:1017402689027592222> | {member.name}** has been successfully removed from the blacklist",
            color=discord.Colour(0xFF1B1B))
        with open("blacklist.json") as file:
            blacklist = json.load(file)
            embed.set_footer(
...
async def pfps(ctx):
    a = requests.get(
        'https://api.ihatehaters.repl.co/api/gif/pfps?format=text').text
    b = a.replace('"', '')
    av = discord.Embed(title="Random Pfp",
                       color=0xFF1B1B,
                       timestamp=ctx.message.created_at)
    av.set_image(url=b)
...
mainshop = [{
    "name": "Watch",
    "price": 100,
    "description": "Time <:watchtool:1019456724962385930>"
}, {
    "name": "Laptop",
    "price": 1000,
    "description": "Work <:Laptop:1019456878650069002>"
}, {
    "name": "PC",
    "price": 10000,
    "description": "Gaming <:PC:1019456969695838269>"
}, {
    "name": "Ferrari",
    "price": 99999,
    "description": "Sports Car <:ferrari:1019456103483981865>"
}]
...
async def beg(ctx):
    ...
    await ctx.send(embed=discord.Embed(
        title="begging!",
        description=f"{ctx.author.mention} Got {earnings} coins!",
        color=0xFF1B1B))
...
async def work_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.send('Try again in <t:{}:R>'.format(
...
async def withdraw(ctx, amount=None):
    await open_account(ctx.author)
    if amount == None:
        await ctx.send(embed=discord.Embed(title="Please enter the amount",
                                           color=0xFF1B1B))
        return
    ...
    if amount > bal[1]:
        await ctx.send(embed=discord.Embed(
            title="You do not have sufficient balance", color=0xFF1B1B))
        return
    if amount < 0:
        await ctx.send(embed=discord.Embed(title="Amount must be positive!",
                                           color=0xFF1B1B))
        return
    ...
    await ctx.send(embed=discord.Embed(
        title="withdraw",
        description=f"{ctx.author.mention} You withdraw {amount} coins",
        color=0xFF1B1B))
...
async def deposit(ctx, amount=None):
    ...
    await ctx.send(embed=discord.Embed(
        title=f"{ctx.author.mention} You deposited {amount} coins",
        color=0xFF1B1B))
...
async def send(ctx, member: discord.Member, amount=None):
    ...
    await ctx.send(embed=discord.Embed(
        title=f"{ctx.author.mention} You gave {member} {amount} coins",
        color=0xFF1B1B))
...
async def rob(ctx, member: discord.Member):
    ...
    if bal[0] < 100:
        await ctx.send(embed=discord.Embed(title="It is useless to rob him!",
                                           color=0xFF1B1B))
        return
...
async def slots(ctx, amount=None):
    ...
    if amount < 0:
        await ctx.send(embed=discord.Embed(
            title="Amount'Amount must be positive!", color=0xFF1B1B))
        return
    final = []
    for i in range(3):
    ...
    if final[0] == final[1] or final[1] == final[2] or final[0] == final[2]:
        ...
        await ctx.reply(embed=discord.Embed(
            title=f'You won :) {ctx.author.name}', color=0xFF1B1B))
    else:
        await update_bank(ctx.author, -1 * amount)
        await ctx.reply(embed=discord.Embed(
            title=f'You lose :( {ctx.author.name}', color=0xFF1B1B))
...
async def buy(ctx, item, amount=1):
    ...
    if not res[0]:
        if res[1] == 1:
            await ctx.send(embed=discord.Embed(
                title="That Object isn't there!", color=0xFF1B1B))
            return
        if res[1] == 2:
            await ctx.send(embed=discord.Embed(
                title=
    ...
    await ctx.send(embed=discord.Embed(
        title=f"You just bought {amount} {item}", color=0xFF1B1B))
...
async def sell(ctx, item, amount=1):
    ...
    if not res[0]:
        ...
        if res[1] == 2:
            await ctx.send(embed=discord.Embed(
                title=f"You don't have {amount} {item} in your bag",
                color=0xFF1B1B))
            return
        if res[1] == 3:
            await ctx.send(embed=discord.Embed(
                title=f"You don't have {item} in your inventory",
                color=0xFF1B1B))
            return

    await ctx.send(embed=discord.Embed(title=f"You just sold {amount} {item}",
                                       color=0xFF1B1B))
...
async def ticdelete(ctx):
    await ctx.send(
        f"<:ri8:1038487759750438912> | deleting {ctx.channel.mention} in 1sec."
    )
...
async def delete_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        await ctx.reply(
            "<:Wrong:1017402708703064144> | You are missing Administrator permission(s) to run this command.",
            mention_author=False)
...
async def adduser(ctx, member: discord.Member, channel=None):
    ...
    await ctx.reply(f"Successfully added {member.mention} to {channel}",
                    mention_author=False)
...
async def close(ctx, channel: discord.TextChannel = None):
    ...
    await ctx.reply(
        f'<:ri8:1038487759750438912> | Successfully closed {ctx.channel.mention}',
        mention_author=False)
...
async def roleinfo(ctx, role: discord.Role = None):
    riembed = discord.Embed(title=f"**{role.name}'s Information**",
                            colour=discord.Colour(0xFF1B1B))
    perms = ""
    if role.permissions.administrator:
        perms += "Administrator, "
...
async def unhideall(ctx):
    ...
    if me.top_role >= ctx.message.author.top_role and ctx.message.author.id != ctx.guild.owner_id:
        ...
        return
        await ctx.reply(
            embed=discord.Embed(title="Successfully unhide all channels",
                                description=f"responsible {ctx.author}",
                                color=0x2f3136))
...
async def hideall(ctx):
    ...
    if me.top_role >= ctx.message.author.top_role and ctx.message.author.id != ctx.guild.owner_id:
        ...
        return
        await ctx.reply(
            embed=discord.Embed(title="successfully hide all channels",
                                description=f"responsible {ctx.author}",
                                color=0x2f3136))
...
async def hide(ctx, channel: discord.TextChannel = None):
    ...
    if channel == None:
        ...
        await ctx.reply(embed=discord.Embed(
            title=
            f"<:Icons_correct:1017402689027592222> | <#{channel.id}> is now hidden from the default role.",
            color=0xFF1B1B))
...
async def unhide(ctx, channel: discord.TextChannel = None):
    ...
    if channel == None:
        ...
        await ctx.reply(embed=discord.Embed(
            title=
            f'<:Icons_correct:1017402689027592222> | <#{channel.id}> is now visible to the default role.',
            color=0xFF1B1B))
...
async def vote(ctx):
    author = ctx.message.author
    b = Button(label='Vote Me',
               style=discord.ButtonStyle.link,
               url='https://discordbotlist.com/bots/zenox-5355/upvote')
    view = View()
    view.add_item(b)
    embed = discord.Embed(description=f"click on button below!",
                          color=0xFF1B1B)
...
async def invite(ctx):
    ...
    b = Button(
        ...
        style=discord.ButtonStyle.link,
        url=
        'https://discord.com/api/oauth2/authorize?client_id=863262252211765248&permissions=8&scope=bot'
    )
    view = View()
    view.add_item(b)
...
async def ping(ctx):
    ...
    embed = discord.Embed(
        description=
        f"<a:th_heartbeat:1017469364712263691> Pong! {round(bot.latency * 1000)}ms",
        color=0xFF1B1B)
    embed.set_author(name=f"{author.name}", icon_url=author.display_avatar.url)
    embed.set_thumbnail(url=author.display_avatar.url)
...
async def embed(ctx, *, mesg=f"Format : embed [words]"):
    ...
    await ctx.message.delete()
    embed = discord.Embed(description=mesg,
                          color=0xFF1B1B,
                          timestamp=ctx.message.created_at)
    embed.set_author(name=f"{ctx.author.name}",
                     icon_url=ctx.author.display_avatar.url)
        ...
        if ctx.author.id == ctx.guild.owner_id:
            for channel in ctx.message.guild.channels:
                if channel.name == channeltodelete:
                    ...
                    await ctx.reply(embed=discord.Embed(
                        title=
                        f"<:Icons_correct:1017402689027592222> | Successfully Deleted All Channels With Name Of {channeltodelete}",
                        color=0x2f3136))
    ...
    if me.top_role >= ctx.message.author.top_role and ctx.message.author.id != ctx.guild.owner_id:
        ...
        for role in ctx.message.guild.roles:
            if role.name == roletodelete:
                ...
                await ctx.reply(embed=discord.Embed(
                    title=
                    f"<:Icons_correct:1017402689027592222> | Successfully Deleted All Roles With Name Of {roletodelete}",
                    color=0x00ffff))
...
async def addchannel(ctx, *names):
    ...
    if me.top_role >= ctx.message.author.top_role and ctx.message.author.id != ctx.guild.owner_id:
        ...
        for name in names:
            ...
            await ctx.send(embed=discord.Embed(
                title=
                f'<:Icons_correct:1017402689027592222> | {name} has been created',
                color=0xFF1B1B))
...
async def delchannel(ctx, *channels: discord.TextChannel):
    ...
    if me.top_role >= ctx.message.author.top_role and ctx.message.author.id != ctx.guild.owner_id:
        ...
        for ch in channels:
            ...
            await ctx.send(embed=discord.Embed(
                title=
                f' <:Icons_correct:1017402689027592222> | {ch.name} has been deleted',
                color=0xFF1B1B))
...
async def banner_error(ctx, error):
    if isinstance(error, commands.CommandInvokeError):
        await ctx.send(embed=discord.Embed(
            title="this user does not have any banner", color=0xFF1B1B))
...
async def source(ctx):
    embed = discord.Embed(
        description=
        f"[soward src](https://github.com/PRINCEOP24/Ethrex-bot-src.git)",
        color=0xFF1B1B)
    # embed.set_author(name=f"{user.avatar}")
    embed.set_thumbnail(
        url=
        "https://images-ext-2.discordapp.net/external/kHT4vIV2yvQVr_TVRWuSfbSPVzLuI0hjxVULZroCx-E/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/863262252211765248/fa3d59466f44e012f70d541d9d409520.png"
    )
...
async def serverslist(ctx, page: int = 1):
    ...
    if 1 <= page <= pages:
        ...
        await ctx.send(embed=discord.Embed(description='Invalid Page Number.'),
                       delete_after=10)
...
async def setprefix(ctx, prefixx):
    ...
    if len(prefixx) > 5:
        await ctx.reply(embed=discord.Embed(
            color=discord.Colour(0xFF1B1B),
            description=
            f'<:Wrong:1017402708703064144> | Prefix Cannot Exceed More Than 5 Letters'
        ))
    elif len(prefixx) <= 5:
        ...
        await ctx.reply(embed=discord.Embed(
            color=discord.Colour(0xFF1B1B),
            description=
            f'<:Icons_correct:1017402689027592222> | Updated Server Prefix To `{prefixx}`'
        ))
